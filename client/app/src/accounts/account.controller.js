  ...
  function AccountController(
    ...
    self.setLanguage = function() {
      ...
      self.languages = [
        ...
        {
          name: gettextCatalog.getString("Portuguese - Portugal"),
          code: "pt_PT"
        },
    ...
    electron.ipcRenderer.on("uri", (event, uri) => {
      $timeout(() => {
        ...
        if (scheme && !self.selected)
          return toastService.error(
            gettext("Select an account and open the URI again")
          );
        if (scheme && self.selected)
          return $scope.$broadcast("app:onURI", scheme);
      }, 0);
    });
      ...
      const confirm = $mdDialog
        ...
        .textContent(
          gettextCatalog.getString(
            "All your data, including created accounts, networks and contacts will be removed from the app and reset to default."
          )
        )
    ...
    const getLatestClientVersion = () => {
      return new Promise((resolve, reject) => {
        const url =
          "https://api.github.com/repos/ArkEcosystem/ark-desktop/releases/latest";
        $http.get(url, { timeout: 5000 }).then(
    ...
    self.clientVersion = require(_path.resolve(
      __dirname,
      "../../package.json"
    )).version;
    ...
    self.accountRefreshState = utilityService.createRefreshState(
      gettext("Account refreshed"),
      gettext("Could not refresh account")
    );
    ...
    self.refreshAccountsAutomatically =
      storageService.get("refreshAccountsAutomatically") || false;
    self.playFundsReceivedSound =
      storageService.get("playFundsReceivedSound") || false;
    ...
    self.switchNetwork = (newNetwork, reload) => {
      if (reload) {
        dialogService.openLoadingDialog(
          self.currentTheme,
      ...
      connectedPeer => {
        ...
        $timeout(() => {
          if (!self.connectedPeer.isConnected && self.isNetworkConnected) {
            ...
            self.connectedPeer.isConnected &&
    ...
    self.toggleBitcoinCurrency = function(force) {
      ...
      self.toggleCurrency = self.btcValueActive
        ? self.currency
        : self.bitcoinCurrency;
    };
    ...
    self.selectCurrency = function($event) {
      ...
      if ($event.shiftKey) {
        ...
        newIndex =
          currencyIndex === 0 ? currenciesNames.length - 1 : currencyIndex - 1;
      } else {
        // Select the next currency
        newIndex =
          currencyIndex === currenciesNames.length - 1 ? 0 : currencyIndex + 1;
      }
    ...
    self.saveFolder = function(account, folder) {
      accountService.setToFolder(
        account.address,
        folder,
        utilityService.arkToArktoshi(account.virtual.uservalue(folder)())
      );
    };

    self.deleteFolder = function(account, foldername) {
      account.virtual = accountService.deleteFolder(
        account.address,
        foldername
      );
    };
    ...
    self.manageFolder = function(account, currentFolderName) {
      const titleText = !currentFolderName
        ? gettext("Create Virtual Folder")
        : gettext("Rename Virtual Folder");
      const buttonText = !currentFolderName ? gettext("Add") : gettext("Save");
      const confirmText = !currentFolderName
        ? gettext("Virtual folder added!")
        : gettext("Virtual folder saved!");
      ...
      if (account.virtual) {
        ...
        $mdDialog.show(confirm).then(foldername => {
          if (account.virtual[foldername]) {
            ...
            if (!currentFolderName) {
              account.virtual = accountService.setToFolder(
                account.address,
                foldername,
                0
              );
            } else {
              account.virtual = accountService.renameFolder(
                account.address,
                currentFolderName,
                foldername
              );
        ...
        confirm = $mdDialog
          ...
          .textContent(
            gettextCatalog.getString(
              "Please enter the passphrase of this account to proceed."
            )
          )
        ...
        $mdDialog.show(confirm).then(passphrase => {
          accountService.createVirtual(passphrase).then(
            virtual => {
            ...
            err => {
              toastService.success(
                gettextCatalog.getString("Error when trying to login:") +
                  " " +
                  err,
                3000,
                true
    ...
    function gotoAddress(address) {
      ...
      accountService.fetchAccountAndForget(currentaddress).then(a => {
        ...
        accountService.refreshAccount(self.selected).then(account => {
        ...
        accountService.getTransactions(currentaddress).then(transactions => {
          if (self.selected.address === currentaddress) {
            if (!self.selected.transactions) {
              ...
              if (
                previousTx.length &&
                !previousTx[0].confirmations &&
                previousTx[0].id !== transactions[0].id
              ) {
            ...
            $timeout(() => {
              $scope.$broadcast(
                "account:onRefreshTransactions",
                self.selected.transactions
              );
            });
          }
        });
        ...
        accountService.getDelegate(self.selected.publicKey).then(delegate => {
          if (self.selected.address === currentaddress) {
    ...
    function refreshCurrentAccount(showToast) {
      ...
      accountService
        ...
        .finally(() => {
          ...
          self.accountRefreshState.updateRefreshState(
            showToast ? toastService : null
          );
        });
      accountService
          ...
          if (self.selected.address === myaccount.address) {
            if (!self.selected.transactions) {
              ...
              if (
                ...
                transactions[0].recipientId === myaccount.address
              ) {
                const wavFile = _path.resolve(
                  __dirname,
                  "assets/audio/power-up.wav"
                );
    ...
    self.searchContactOrAccount = (text, exactMatch) => {
      ...
      const accounts = self.getAllAccounts().map(acc => {
        return {
          name: acc.username,
          address: acc.address,
          type: gettext("Account"),
          icon: acc.icon
        };
      });
      let contacts = (storageService.get("contacts") || []).map(c => {
        return {
          name: c.name,
          address: c.address,
          type: gettext("Contact"),
          icon: "account_circle"
        };
      });
      ...
      return contacts.filter(
        contact => compareFunc(contact.address) || compareFunc(contact.name)
      );
    };
      ...
      accountService.getTransactions(currentaddress).then(transactions => {
        if (self.selected.address === currentaddress) {
          if (!self.selected.transactions) {
            ...
            if (
              playSound === true &&
              transactions.length > previousTx.length &&
              transactions[0].type === 0 &&
              transactions[0].recipientId === self.selected.address
            ) {
              const wavFile = _path.resolve(
                __dirname,
                "assets/audio/power-up.wav"
    ...
    function addWatchOnlyAddress() {
      function validateAddress() {
        ...
        if (isAddress.test(address)) {
          ...
          toastService.error(
            gettextCatalog.getString(
              "Address '{{ address }}' is not recognized",
              { address: address }
            ),
            3000,
            true
    ...
    function getAllDelegates(selectedAccount) {
      ...
      if (selectedAccount.selectedVotes) {
        return arrayUnique(
          selectedAccount.selectedVotes.concat(selectedAccount.delegates)
        );
    ...
    function generateDynamicPalette(callback) {
      ...
      vibrant.from(url).getPalette((err, palette) => {
        ...
        Object.keys(materialPalette).forEach(color => {
          const vibrantDiff = vibrant.Util.hexDiff(
            materialPalette[color]["900"]["hex"],
            palette.Vibrant.getHex()
          );
          vibrantRatio[color] = vibrantDiff;

          const darkVibrantDiff = vibrant.Util.hexDiff(
            materialPalette[color]["900"]["hex"],
    ...
    function manageBackgrounds() {
      ...
      for (const folder in backgrounds) {
        ...
        if (fs.existsSync(_path.resolve(fullPath))) {
          // check dir exists
          const image = {};
      ...
      function upload() {
        ...
        require("electron").remote.dialog.showOpenDialog(options, fileName => {
          ...
          readStream
            .on("readable", () => {
              toastService.success(
                gettext("Background added successfully!"),
                3000
              );
            ...
            .on("error", error => {
              toastService.error(
                `Error Adding Background (reading): ${error}`,
                3000
              );
            })
            .on("error", error => {
              toastService.error(
                gettextCatalog.getString("Error adding background:") +
                  " " +
                  error,
                3000
              );
    ...
    function manageNetworks() {
      ...
      function createNetwork() {
        networkService
          .createNetwork($scope.send.createnetwork)
          .then(newNetwork => {
      ...
      function removeNetwork(network) {
        ...
        const confirm = $mdDialog
          .confirm()
          .title(
            gettextCatalog.getString("Remove network '{{ network }}'", {
              network: network
            })
          )
          .theme(self.currentTheme)
          .textContent(
            gettextCatalog.getString(
              "Are you sure you want to remove this network and all data (accounts and settings) associated with it from your computer. Your accounts are still safe on the blockchain."
            )
          )
          .ok(
            gettextCatalog.getString(
              "Remove from my computer all cached data from this network"
            )
          )
          .cancel(gettextCatalog.getString("Cancel"));
        $mdDialog.show(confirm).then(() => {
          ...
          if (isActive) {
            $mdDialog.show({
              ...
              controller: [
                ...
                "switchNetwork",
                "theme",
                ($scope, networkName, switchNetwork, theme) => {
    ...
    function createDelegate(selectedAccount) {
      ...
      function next() {
        ...
        let delegateName;
        try {
          delegateName = accountService.sanitizeDelegateName(
            $scope.createDelegate.data.username
          );
        } catch (error) {
    ...
    function createAccount() {
      ...
      const data = {
        passphrase: bip39.generateMnemonic(
          null,
          null,
          bip39.wordlists[self.getWordlistLanguage()]
        )
      };

      function next() {
        if (!$scope.createAccountDialog.data.showRepassphrase) {
          $scope.createAccountDialog.data.repassphrase =
          ...
          if (
            $scope.createAccountDialog.data.word3 === words[2] &&
            $scope.createAccountDialog.data.word6 === words[5] &&
            $scope.createAccountDialog.data.word9 === words[8]
          ) {
            accountService
              .createAccount($scope.createAccountDialog.data.repassphrase)
              .then(account => {
                self.accounts.push(account);
                toastService.success(
                  gettextCatalog.getString(
    ...
    function importAccount() {
      ...
      function save() {
        ...
        if (
          !$scope.send.data.customPassphrase &&
          !isBIP39($scope.send.data.passphrase)
        ) {
          toastService.error(
            gettextCatalog.getString(
              "Not a valid BIP39 passphrase! Please check all words and spaces."
            ),
            null,
            true
          );
        ...
        accountService
          .createAccount($scope.send.data.passphrase)
          .then(account => {
            ...
            for (let i = 0; i < self.accounts.length; i++) {
              if (self.accounts[i].address === account.address) {
                toastService.error(
                  gettextCatalog.getString(
                    "Account '{{ address }}' has already been imported!",
                    { address: account.address }
                  ),
                  null,
                  true
            ...
            toastService.success(
              gettextCatalog.getString(
                "Account '{{ address }}' successfully imported!",
                { address: account.address }
              ),
              null,
              true
    ...
    function createSecondPassphrase(selectedAccount) {
      ...
      if (selectedAccount.secondSignature) {
        return formatAndToastError(
          gettextCatalog.getString(
            "The account '{{ address }}' already has a second passphrase!",
      ...
      function warnAboutSecondPassphraseFee() {
        accountService.getFees(true).then(fees => {
          ...
          const secondPhraseArkVal = utilityService.arktoshiToArk(
            secondPhraseArktoshiVal,
            true
          );
          const confirm = $mdDialog.confirm({
            title: gettextCatalog.getString(
              "Second Passphrase fee ({{ currency }})",
              { currency: networkService.getNetwork().symbol }
            ),
            secondPhraseArkVal: secondPhraseArkVal,
            textContent: gettextCatalog.getString(
              "WARNING! Second passphrase creation costs {{ cost }} {{ currency }}",
              {
                cost: secondPhraseArkVal,
      ...
      function next() {
        if (!$scope.createSecondPassphraseDialog.data.showRepassphrase) {
          $scope.createSecondPassphraseDialog.data.reSecondPassphrase =
          ...
          transactionBuilderService
            .createSecondPassphraseCreationTransaction({
              fromAddress: selectedAccount.address,
              masterpassphrase:
                $scope.createSecondPassphraseDialog.data.passphrase,
              secondpassphrase:
                $scope.createSecondPassphraseDialog.data.reSecondPassphrase
            })
            .then(transaction => {
    ...
    function loadSignedMessages() {
      self.selected.signedMessages = storageService.get(
        "signed-" + self.selected.address
      );
    }
    ...
    function showValidateTransaction(selectedAccount, transaction, cb) {
        ...
        require("electron").remote.dialog.showSaveDialog(
          ...
          fileName => {
            ...
            fs.writeFile(fileName, raw, "utf8", err => {
              if (err) {
                toastService.error(
                  gettextCatalog.getString("Failed to save transaction file") +
                    ": " +
                    err,
                  null,
                  true
                ...
                toastService.success(
                  gettextCatalog.getString(
                    "Transaction file successfully saved in '{{ fileName }}'.",
                    { fileName: fileName }
                  ),
                  null,
                  true
      ...
      function send() {
        ...
        transaction = accountService.formatTransaction(
          transaction,
        ...
        networkService.postTransaction(transaction).then(
          transaction => {
            ...
            toastService.success(
              gettextCatalog.getString(
                "Transaction '{{ transactionId }}' sent with success!",
          ...
          error => {
            const message = gettextCatalog.getString(
              "Failed to execute your '{{ transactionLabel }}' transaction!",
              { transactionLabel }
            );
      ...
      $scope.validate = {
        ...
        totalAmount: utilityService.arktoshiToArk(
          parseFloat(transaction.amount) + transaction.fee,
          true
        )
      };

      const contacts = self.searchContactOrAccount(
